...
- **Renombrado (rename)**: este patrón nos indica que debemos cambiar el nombre de un paquete, clase, método o campo, por un nombre más significativo. Por ejemplo, supungamos el siguiente código:
  ```java
  // ¿Qué es 'd'? ¿Días? ¿Distancia?
  int d = 86400; 
  void prnt(String s) { ... }
  ```
  
  Vemos que la variable `d`no ofrece información sobre el valor que contiene. Deberíamos modificar su nombre para que diese información más relevante. En nuestro caso, el código debería ser el siguiente:
